...
type Order = {
  ...
  tableNumber?: string
  createdAt: string
  items: { id: string; name: string; quantity: number; note?: string; serveFor: 'to_table'|'to_go'; status: string }[]
}
...
export default function EmployeePage() {
  ...
  async function updateItem(id: string, status: string) {
    await fetch('/api/orders/item', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) })
    load()
  }
  ...
  return (
    <div>
      ...
      <p className="opacity-80">Actualiza estados (pendiente → preparando → listo → servido).</p>
                ...
                <div key={it.id} className="border rounded-xl p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {it.note && <div className="text-sm opacity-70">Nota: {it.note}</div>}
                      <div className="text-xs opacity-60">Entrega: {it.serveFor === 'to_go' ? 'Para llevar' : 'A mesa'}</div>
                    </div>
                    <div className="flex gap-2">
                      <button className="btn btn-outline" onClick={() => updateItem(it.id, 'preparing')}>Preparando</button>
                      <button className="btn btn-primary" onClick={() => updateItem(it.id, 'ready')}>Listo</button>
                    </div>
                  </div>
                </div>
              ))}
